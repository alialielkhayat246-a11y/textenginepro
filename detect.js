...
// Runs franc server-side. Called by the frontend via fetch('/api/detect').

import { franc } from 'franc-min';

// ISO 639-3 → display info
const LANG_MAP = {
  ara: { name: 'Arabic',             native: 'العربية',        flag: '🇸🇦', family: 'Semitic',       script: 'Arabic'     },
  arb: { name: 'Arabic (MSA)',        native: 'العربية الفصحى', flag: '🇸🇦', family: 'Semitic',       script: 'Arabic'     },
  ...
  som: { name: 'Somali',              native: 'Soomaali',       flag: '🇸🇴', family: 'Cushitic',      script: 'Latin'      },
  ...
  ltz: { name: 'Luxembourgish',       native: 'Lëtzebuergesch', flag: '🇱🇺', family: 'Germanic',      script: 'Latin'      },
  ...
  lao: { name: 'Lao',                 native: 'ພາສາລາວ',         flag: '🇱🇦', family: 'Tai-Kadai',     script: 'Lao'        },
  ...
  mya: { name: 'Burmese',             native: 'မြန်မာဘာသာ',     flag: '🇲🇲', family: 'Sino-Tibetan',  script: 'Myanmar'    },
  lat: { name: 'Latin',               native: 'Latina',         flag: '🏛️',  family: 'Italic',        script: 'Latin'      },
  hye: { name: 'Armenian',            native: 'Հայերեն',        flag: '🇦🇲', family: 'IE Isolate',    script: 'Armenian'   },
...
export default function handler(req, res) {
  ...
  // franc detects with full trigram model on the server
  let code = franc(text, { minLength: 3 });
  if (code === 'und') code = franc(text, { minLength: 1 });

  const info = LANG_MAP[code] || null;

  return res.status(200).json({
    code,                       // ISO 639-3 code e.g. "ara"
    name:   info?.name   || code,
    native: info?.native || '',
    flag:   info?.flag   || '🌐',
